...
export const MainActions = ({serviceProviders, status}) => {
    ...
    return (
        <Grid item xs={12} md={12} lg={4} className="action-button-grid">
            ...
            <PopupComponent buttonLabel="Cancel" modalTitle="Cancel" btnClasses={`${classes.actionButton} action-button`} btn2Classes={`${classes.actionButton} action-button`} btn1Classes={`${classes.actionButton} action-button`} btnStartIcon={<Cancel/>} btn1Func={updateWOStatus} MuiDialogTitle={classes.MuiDialogTitle} content="Are you sure you want to change the status to Cancel?" />
        </Grid>
    )
}